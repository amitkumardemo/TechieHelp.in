...
export default function LeadAIAdmin() {
  ...
  const fetchWorkspacesAndLogs = async () => {
    try {
      ...
      profilesSnap.forEach(doc => {
        const p = doc.data();
        if (p.workspaceId) {
          profilesMap[p.workspaceId] = p.email;
        }
      });
      ...
      wsSnap.forEach(doc => {
        ...
        wsList.push({
          ...data,
          ownerEmail: profilesMap[data.id] || "N/A"
        });
      });
      ...
      if (configSnap.exists()) {
        setRequireApproval(configSnap.data().requireAdminApproval ?? true);
      } else {
      ...
      console.error("Failed to fetch workspaces/logs in admin:", e);
    } finally {
      setWsLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (activeTab === "workspaces" || activeTab === "overview") {
      fetchWorkspacesAndLogs();
    }
  }, [activeTab]);

  const handleUpdateWorkspaceStatus = async (wsId, newStatus) => {
    try {
      const wsRef = doc(db, "workspaces", wsId);
      await updateDoc(wsRef, { status: newStatus });
      setWorkspaces(prev => prev.map(ws => ws.id === wsId ? { ...ws, status: newStatus } : ws));

      const logId = "log_" + Date.now() + "_" + Math.random().toString(36).substring(2, 7);
      await setDoc(doc(db, "auditLogs", logId), {
        userId: "admin",
        action: `WORKSPACE_${newStatus.toUpperCase()}`,
        timestamp: new Date().toISOString(),
        userAgent: navigator.userAgent,
        details: { workspaceId: wsId }
      });
      fetchWorkspacesAndLogs();
    } catch (e) {
      console.error("Failed to update workspace status:", e);
    }
  };
  ...
  const handleToggleApprovalSetting = async () => {
    ...
    try {
      await setDoc(doc(db, "config", "system"), { requireAdminApproval: newValue }, { merge: true });
      ...
      await setDoc(doc(db, "auditLogs", logId), {
        ...
        timestamp: new Date().toISOString(),
        userAgent: navigator.userAgent,
        details: { requireAdminApproval: newValue }
      });
      fetchWorkspacesAndLogs();
    } catch (e) {
      console.error("Failed to update approval configuration:", e);
    }
  };
  ...
  const [webhookUrl, setWebhookUrl] = useState("https://api.techiehelp.in/v1/webhooks/LeadAI");
      ...
      <aside className="w-64 border-r border-white/[0.06] bg-[#05050a]/90 backdrop-blur-2xl flex flex-col justify-between h-screen shrink-0 sticky top-0">
        <div>
          ...
          <div className="p-6 flex items-center gap-3 border-b border-white/[0.04]">
            ...
            <div>
              <h1 className="font-bold text-sm tracking-tight text-gray-900 dark:text-white">LeadAI</h1>
          ...
          <nav className="p-4 space-y-1 overflow-y-auto max-h-[calc(100vh-180px)] scrollbar-hide">
            {[
              ...
              { id: "workspaces", label: "B2B Workspaces", icon: Database },
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-xs font-medium transition-all duration-200 group ${isActive
                    ...
                    : "text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:text-white hover:bg-white/[0.02]"
                    }`}
                >
                  ...
                  {item.badge && (
                    <span className="text-[9px] bg-white/[0.06] text-gray-600 dark:text-gray-400 px-1.5 py-0.5 rounded font-mono">
                      {item.badge}
                    </span>
                  )}
                  {item.dot && (
        ...
        <div className="p-4 border-t border-white/[0.04] bg-white/[0.01] flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-xs font-semibold text-gray-900 dark:text-white">Aditya Kumar</p>
          ...
          <a href="/" className="text-gray-600 dark:text-gray-400 hover:text-red-400 transition-colors p-1.5 rounded-lg hover:bg-white/[0.03]">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-y-auto pb-12 relative">
        ...
        <header className="sticky top-0 bg-[#030307]/70 backdrop-blur-md z-40 border-b border-white/[0.05] py-4 px-8 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div>
              <h2 className="text-lg font-bold text-gray-900 dark:text-white tracking-tight">Welcome Back, Admin</h2>
              <p className="text-xs text-gray-600 dark:text-gray-400 font-mono">TechieHelp LeadAI Control Center</p>
            </div>
          ...
          <div className="flex items-center gap-3">
            <div className="text-right">
              <p className="text-xs text-gray-600 dark:text-gray-400">LLM Processing Speed</p>
              <p className="text-[11px] font-mono text-gray-900 dark:text-white font-semibold flex items-center justify-end gap-1">
            ...
            <button
              onClick={() => {
                ...
                toast.innerHTML = `<span class="w-2 h-2 rounded-full bg-green-400 animate-pulse"></span> <span class="text-gray-900 dark:text-white">Metrics refreshed successfully</span>`;
                document.body.appendChild(toast);
                setTimeout(() => toast.remove(), 2500);
              }}
              className="p-2 bg-white/[0.03] hover:bg-white/[0.06] border border-white/[0.06] hover:border-white/[0.1] text-gray-300 hover:text-gray-900 dark:text-white rounded-lg transition-all"
        ...
        <div className="p-8 max-w-7xl w-full mx-auto space-y-8">
          ...
          <AnimatePresence mode="wait">
            ...
            >
                ...
                <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {[
                      ...
                      { title: "Leads Processed", value: stats.leadsProcessed.toLocaleString(), trend: "+24.8%", up: true, subtitle: "scored by LeadAI" },
                      ...
                      <div key={i} className="glass border border-white/[0.05] hover:border-white/[0.1] rounded-2xl p-5 shadow-2xl relative overflow-hidden group transition-all duration-300">
                        ...
                        <div className="relative z-10 flex flex-col justify-between h-full">
                          <div className="flex items-center justify-between text-gray-600 dark:text-gray-400 text-xs font-medium mb-2">
                          ...
                          <div className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight font-mono">{card.value}</div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 glass border border-white/[0.05] rounded-2xl p-6 shadow-2xl flex flex-col justify-between">
                      <div className="flex items-center justify-between border-b border-white/[0.05] pb-4 mb-4">
                        <div className="flex items-center gap-2">
                          ...
                          <h3 className="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider">AI Activity Live Processing Feed</h3>
                        </div>
                          ...
                          {logs.slice(0, 6).map((log) => (
                            ...
                            >
                              <div className="flex items-center gap-3 flex-1 min-w-0">
                                <span className={`w-2 h-2 rounded-full shrink-0 ${log.type === "lead" ? "bg-red-500" :
                      ...
                      <button
                        ...
                        className="w-full text-center text-xs text-[#38bdf8] hover:text-gray-900 dark:text-white mt-4 font-medium transition-colors"
                      >
                    ...
                    <div className="glass border border-white/[0.05] rounded-2xl p-6 shadow-2xl flex flex-col justify-between">
                      <div>
                        <div className="flex items-center justify-between border-b border-white/[0.05] pb-4 mb-4">
                          <h3 className="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider">Lead Funnel qualification</h3>
                            ...
                            <div key={idx} className="relative p-2.5 rounded-xl border border-white/[0.03] overflow-hidden bg-white/[0.01]">
                              ...
                              <div className="flex items-center justify-between text-xs relative z-10">
                                <div>
                                  ...
                                  <p className="text-[10px] text-gray-600 dark:text-gray-400 font-mono mt-0.5">{item.count} total leads</p>
                                </div>
                                <span className="font-mono text-gray-900 dark:text-white text-[11px] font-bold">{item.percent}</span>
                              </div>
                            </div>
                          ))}
                        </div>
                  ...
                  <div className="glass border border-white/[0.05] rounded-2xl p-6 shadow-2xl">
                    <div className="flex items-center justify-between mb-6">
                      <div>
                        <h3 className="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider">Active Customers & API Metrics</h3>
                        <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">Real-time status of LeadAI connected customer environments</p>
                      </div>
                      <button
                        onClick={() => setActiveTab("customers")}
                        className="text-xs text-[#38bdf8] hover:text-gray-900 dark:text-white font-medium transition-colors border border-[#38bdf8]/20 hover:border-[#38bdf8]/50 px-3 py-1.5 rounded-lg bg-[#38bdf8]/5 hover:bg-[#38bdf8]/10"
                      >
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-left border-collapse text-xs">
                        <thead>
                          <tr className="border-b border-white/[0.05] text-gray-600 dark:text-gray-400 font-mono uppercase text-[10px]">
                        ...
                        <tbody className="divide-y divide-white/[0.03]">
                          {customers.slice(0, 4).map((c) => (
                            <tr key={c.id} className="hover:bg-white/[0.01] transition-colors">
                              <td className="py-4 text-gray-900 dark:text-white font-medium">{c.name}</td>
                              <td className="py-4 text-gray-300">{c.company}</td>
                              <td className="py-4">
                                <span className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold ${c.plan === "Enterprise" ? "bg-purple-500/10 text-[#33bbcf] border border-purple-500/20" :
                              ...
                              <td className="py-4 font-mono font-semibold text-gray-900 dark:text-white">{c.apiCalls.toLocaleString()}</td>
                              <td className="py-4 text-gray-600 dark:text-gray-400 font-mono">{c.lastActivity}</td>
                              <td className="py-4 text-right">
                                <div className="flex items-center justify-end gap-2">
                                  <button onClick={() => { setSelectedItem(c); setModalType("view_customer"); }} className="p-1.5 bg-white/[0.03] hover:bg-white/[0.08] rounded text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:text-white" title="View Metrics"><Eye className="w-3.5 h-3.5" /></button>
                                  <button onClick={() => handleOpenEdit(c)} className="p-1.5 bg-white/[0.03] hover:bg-white/[0.08] rounded text-gray-600 dark:text-gray-400 hover:text-[#38bdf8]" title="Edit Configuration"><Edit2 className="w-3.5 h-3.5" /></button>
              ...
              {/* ==========================================
                  1.5. B2B WORKSPACES TAB
                  ========================================== */}
              {activeTab === "workspaces" && (
                <div className="space-y-6">
                  {/* Compliance Policy Status Header */}
                  <div className="glass border border-white/[0.05] p-6 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                      <h3 className="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider flex items-center gap-2">
                        <Shield className="w-4 h-4 text-[#38bdf8]" />
                        B2B Workspaces Compliance Settings
                      </h3>
                      <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">
                        Configure global registration rules and review pending corporate workspaces.
                      </p>
                    </div>
                    <div className="card-glass flex flex-col">
                      <div className="text-left">
                        <p className="text-[10px] text-gray-600 dark:text-gray-400 font-mono">GLOBAL POLICY</p>
                        <p className="text-xs font-semibold text-gray-900 dark:text-white">Require Administrative Approval</p>
                      </div>
                      <button
                        onClick={handleToggleApprovalSetting}
                        className={`w-12 h-6 rounded-full transition-all duration-300 relative border ${requireApproval
                          ? "bg-[#38bdf8]/20 border-[#38bdf8]/40"
                          : "bg-white/[0.04] border-white/[0.08]"
                          }`}
                      >
                        <div
                          className={`w-4 h-4 rounded-full absolute top-0.5 transition-all duration-300 ${requireApproval
                            ? "right-0.5 bg-[#38bdf8]"
                  ...
                  <div className="glass border border-white/[0.05] rounded-2xl p-6 shadow-2xl">
                    <div className="flex items-center justify-between mb-6">
                      <div>
                        <h3 className="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider">Corporate Workspaces Registry</h3>
                        <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">Review, approve, reject, or suspend registered business environments.</p>
                      </div>
                      <button
                        onClick={fetchWorkspacesAndLogs}
                        className="text-xs text-[#38bdf8] hover:text-gray-900 dark:text-white font-medium transition-colors border border-[#38bdf8]/20 hover:border-[#38bdf8]/50 px-3 py-1.5 rounded-lg bg-[#38bdf8]/5 hover:bg-[#38bdf8]/10 flex items-center gap-1.5"
                      >
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-left border-collapse text-xs">
                        <thead>
                          <tr className="border-b border-white/[0.05] text-gray-600 dark:text-gray-400 font-mono uppercase text-[10px]">
                            <th className="pb-3 font-semibold">Business / Website</th>
                            <th className="pb-3 font-semibold">Contact Email</th>
                            <th className="pb-3 font-semibold">Industry / Size</th>
                          ...
                          {wsLoading ? (
                            <tr>
                              <td colSpan="7" className="py-12 text-center text-gray-500">
                                <RefreshCw className="w-6 h-6 mx-auto mb-2 text-gray-600 animate-spin" />
                                <p>Loading workspaces from Firestore...</p>
                              </td>
                            </tr>
                          ) : workspaces.length > 0 ? (
                            workspaces.map((ws) => (
                              <tr key={ws.id} className="hover:bg-white/[0.01] transition-colors">
                                <td className="py-4">
                                  <div className="text-gray-900 dark:text-white font-medium">{ws.businessName}</div>
                                  <a href={ws.websiteUrl} target="_blank" rel="noreferrer" className="text-gray-500 text-[10px] font-mono hover:text-[#38bdf8] flex items-center gap-1 mt-0.5">
                                    {ws.websiteUrl} <ExternalLink className="w-3 h-3" />
                                  </a>
                                </td>
                                <td className="py-4 text-gray-300 font-mono">{ws.ownerEmail || ws.email || "N/A"}</td>
                                <td className="py-4 text-gray-300">
                                  <div>{ws.industry || "N/A"}</div>
                                  <div className="text-gray-500 text-[10px] mt-0.5">{ws.companySize || "N/A"} employees</div>
                                </td>
                                <td className="py-4 text-gray-600 dark:text-gray-400 font-mono">{ws.phone || "N/A"}</td>
                                <td className="py-4 text-gray-500 font-mono">
                                  {ws.createdAt ? new Date(ws.createdAt).toLocaleDateString() : "N/A"}
                                </td>
                                <td className="py-4">
                                  <span className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold ${ws.status === "Approved" ? "bg-green-500/10 text-green-400 border border-green-500/20" :
                                    ws.status === "Pending Review" ? "bg-amber-500/10 text-amber-400 border border-amber-500/20 animate-pulse" :
                                      ws.status === "Suspended" ? "bg-red-500/10 text-red-400 border border-red-500/20" :
                                        "bg-gray-500/10 text-gray-600 dark:text-gray-400 border border-gray-500/20"
                                    }`}>
                                    {ws.status}
                                  </span>
                                ...
                                <td className="py-4 text-right">
                                  <div className="flex items-center justify-end gap-2">
                                    {ws.status !== "Approved" && (
                                      <button
                                        onClick={() => handleUpdateWorkspaceStatus(ws.id, "Approved")}
                                        className="px-2.5 py-1 bg-green-500/10 hover:bg-green-500 text-green-400 hover:text-black rounded text-[10px] font-semibold border border-green-500/20 transition-all"
                                      >
                                        Approve
                                      </button>
                                    )}
                                    {ws.status === "Pending Review" && (
                                      <button
                                        onClick={() => handleUpdateWorkspaceStatus(ws.id, "Rejected")}
                                        className="px-2.5 py-1 bg-red-500/10 hover:bg-red-500 text-red-400 hover:text-gray-900 dark:text-white rounded text-[10px] font-semibold border border-red-500/20 transition-all"
                                      >
                                        Reject
                                      </button>
                                    )}
                                    {ws.status === "Approved" && (
                                      <button
                                        ...
                                        className="px-2.5 py-1 bg-amber-500/10 hover:bg-amber-500 text-amber-400 hover:text-black rounded text-[10px] font-semibold border border-amber-500/20 transition-all"
                                      >
                                        Suspend
                                      </button>
                                    )}
                            ...
                            <tr>
                              <td colSpan="7" className="py-12 text-center text-gray-500">
                                ...
                                <p>No business workspaces registered yet.</p>
                              </td>
                            </tr>
                          )}
                        </tbody>
                  ...
                  <div className="glass border border-white/[0.05] rounded-2xl p-6 shadow-2xl">
                    <div className="flex items-center justify-between mb-6 border-b border-white/[0.05] pb-4">
                      <div>
                        ...
                        <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">Immutable ledger tracking logins, signups, lockouts, and admin actions.</p>
                      </div>
                    ...
                    <div className="space-y-3 max-h-[400px] overflow-y-auto pr-1">
                      {auditLogs.length > 0 ? (
                        auditLogs.map((log) => (
                          <div
                            key={log.id}
                            className="p-3.5 bg-white/[0.01] border border-white/[0.03] rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-xs hover:bg-white/[0.02] transition-all"
                          >
                            <div className="flex-1 space-y-1">
                              <div className="flex items-center gap-2">
                                <span className={`w-2 h-2 rounded-full ${log.action?.includes("SUCCESS") || log.action?.includes("APPROVED") ? "bg-green-400" :
                                  log.action?.includes("FAILED") || log.action?.includes("REJECTED") || log.action?.includes("LOCKOUT") ? "bg-red-400" :
                                    log.action?.includes("SUSPEND") ? "bg-amber-400" : "bg-[#38bdf8]"
                                  }`} />
                                <span className="font-mono font-bold text-gray-900 dark:text-white">{log.action}</span>
                                <span className="text-[10px] text-gray-500 font-mono">User: {log.userId}</span>
                              </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 glass border border-white/[0.05] p-5 rounded-2xl">
                    <div className="relative flex-1 max-w-md">
                      ...
                      <input
                        ...
                        className="w-full pl-10 pr-4 py-2.5 bg-white/[0.03] border border-white/[0.06] focus:border-[#38bdf8]/50 outline-none rounded-xl text-xs text-gray-900 dark:text-white transition-all font-sans"
                      />
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <select
                        ...
                        className="bg-white/[0.03] border border-white/[0.06] text-xs text-gray-900 dark:text-white rounded-lg p-2 outline-none cursor-pointer"
                      >
                          ...
                          {filteredCustomers.length > 0 ? (
                            filteredCustomers.map((c) => (
                              <tr key={c.id} className="hover:bg-white/[0.01] transition-colors">
                                ...
                                <td className="py-4 font-mono font-semibold text-gray-900 dark:text-white">{c.workflowsCount} Active</td>
                                <td className="py-4 text-center">
                                  {c.gmailConnected ? (
                                ...
                                <td className="py-4 font-mono text-gray-900 dark:text-white">{c.apiCalls.toLocaleString()}</td>
                                <td className="py-4">
                                  <span className={`flex items-center gap-1.5 text-[11px] ${c.status === "Active" ? "text-green-400" : "text-red-400"}`}>
                                    <span className={`w-1.5 h-1.5 rounded-full ${c.status === "Active" ? "bg-green-400" : "bg-red-400 animate-pulse"}`} />
                                    {c.status}
                                ...
                                <td className="py-4 text-right">
                                  <div className="flex items-center justify-end gap-2">
                                    <button onClick={() => { setSelectedItem(c); setModalType("view_customer"); }} className="p-1.5 bg-white/[0.03] hover:bg-white/[0.08] rounded text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:text-white" title="View Stats"><Eye className="w-3.5 h-3.5" /></button>
                                    <button onClick={() => handleOpenEdit(c)} className="p-1.5 bg-white/[0.03] hover:bg-white/[0.08] rounded text-gray-600 dark:text-gray-400 hover:text-[#38bdf8]" title="Edit Tier"><Edit2 className="w-3.5 h-3.5" /></button>
                                    <button onClick={() => handleToggleSuspend(c)} className={`p-1.5 bg-white/[0.03] hover:bg-white/[0.08] rounded ${c.status === "Active" ? "text-red-400 hover:bg-red-500/10" : "text-green-400 hover:bg-green-500/10"}`} title={c.status === "Active" ? "Suspend" : "Activate"}>
                                      {c.status === "Active" ? <UserMinus className="w-3.5 h-3.5" /> : <UserCheck className="w-3.5 h-3.5" />}
                                    </button>
                                  </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div className="glass border border-white/[0.05] p-5 rounded-2xl">
                      <p className="text-gray-600 dark:text-gray-400 text-xs font-semibold">Emails Analyzed</p>
                      <h4 className="text-2xl font-bold font-mono text-gray-900 dark:text-white mt-1">104,230</h4>
                      <p className="text-[9px] font-mono text-green-400 mt-2 flex items-center gap-0.5">
                        <ArrowUpRight className="w-3 h-3" /> +15.2% MoM
                      </p>
                    </div>
                    <div className="glass border border-white/[0.05] p-5 rounded-2xl">
                      <p className="text-gray-600 dark:text-gray-400 text-xs font-semibold">AI Generated Drafts</p>
                      <h4 className="text-2xl font-bold font-mono text-gray-900 dark:text-white mt-1">89,432</h4>
                      <p className="text-[9px] font-mono text-green-400 mt-2 flex items-center gap-0.5">
                        <ArrowUpRight className="w-3 h-3" /> +18.2% MoM
                      </p>
                    </div>
                    <div className="glass border border-white/[0.05] p-5 rounded-2xl">
                      <p className="text-gray-600 dark:text-gray-400 text-xs font-semibold">Response Send Rate</p>
                      <h4 className="text-2xl font-bold font-mono text-gray-900 dark:text-white mt-1">85.8%</h4>
                      <p className="text-[9px] font-mono text-gray-500 mt-2">Auto-submitted replies</p>
                    </div>
                    <div className="glass border border-white/[0.05] p-5 rounded-2xl">
                      <p className="text-gray-600 dark:text-gray-400 text-xs font-semibold">Average Qualification Score</p>
                      <h4 className="text-2xl font-bold font-mono text-gray-900 dark:text-white mt-1">84.2/100</h4>
                      <p className="text-[9px] font-mono text-green-400 mt-2 flex items-center gap-0.5">
                  ...
                  <div className="glass border border-white/[0.05] p-6 rounded-2xl">
                    <h3 className="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider mb-6">AI Automated Email Volume (Last 7 Days)</h3>
                    ...
                    <div className="flex items-center gap-6 mt-4 text-[10px] font-mono text-gray-600 dark:text-gray-400 justify-center">
                  ...
                  {/* Recent Emails Queue */}
                  <div className="glass border border-white/[0.05] p-6 rounded-2xl">
                    <h3 className="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider mb-4">Live Email Draft Ingestion Queue</h3>
                    <div className="overflow-x-auto">
                      <table className="w-full text-left border-collapse text-xs">
                        <thead>
                          <tr className="border-b border-white/[0.05] text-gray-600 dark:text-gray-400 font-mono uppercase text-[10px]">
                        ...
                        <tbody className="divide-y divide-white/[0.03]">
                          {RECENT_EMAILS.map((email) => (
                            <tr key={email.id} className="hover:bg-white/[0.01]">
                              <td className="py-4 text-gray-900 dark:text-white font-medium">{email.customer}</td>
                              <td className="py-4 text-gray-300 font-mono">{email.leadEmail}</td>
                              <td className="py-4 text-gray-600 dark:text-gray-400 truncate max-w-xs">{email.subject}</td>
                              <td className="py-4 font-mono font-semibold">
                                <span className={`px-2 py-0.5 rounded ${email.score >= 80 ? "bg-green-500/10 text-green-400" :
                              ...
                              <td className="py-4">
                                <span className={`px-2 py-0.5 rounded text-[10px] font-semibold ${email.status === "Qualified" ? "bg-green-500/10 text-green-400" :
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="glass border border-white/[0.05] p-6 rounded-2xl flex items-center justify-between">
                      <div>
                        <p className="text-gray-600 dark:text-gray-400 text-xs font-semibold uppercase">Hot Leads (Score 80-100)</p>
                    ...
                    <div className="glass border border-white/[0.05] p-6 rounded-2xl flex items-center justify-between">
                      <div>
                        <p className="text-gray-600 dark:text-gray-400 text-xs font-semibold uppercase">Warm Leads (Score 50-79)</p>
                    ...
                    <div className="glass border border-white/[0.05] p-6 rounded-2xl flex items-center justify-between">
                      <div>
                        <p className="text-gray-600 dark:text-gray-400 text-xs font-semibold uppercase">Cold Leads (Score &lt; 50)</p>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 glass border border-white/[0.05] p-6 rounded-2xl">
                      <h3 className="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider mb-6">Top Performing Customer Workspaces</h3>
                        ...
                        {[
                          ...
                          <div key={idx} className="space-y-2">
                            <div className="flex items-center justify-between text-xs">
                              <div>
                                <span className="text-gray-900 dark:text-white font-medium">{cust.company}</span>
                    ...
                    <div className="glass border border-white/[0.05] p-6 rounded-2xl flex flex-col justify-between">
                      <div>
                        <h3 className="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider mb-6">LeadAI Accuracy Index</h3>
                        <div className="flex items-center justify-center py-4 relative">
                          ...
                          <div className="absolute text-center">
                            <p className="text-xl font-bold font-mono text-gray-900 dark:text-white">91.2%</p>
                            <p className="text-[8px] text-gray-500 uppercase tracking-widest font-mono mt-0.5">Confidence</p>
                          </div>
                        </div>
                        <p className="text-[11px] text-gray-600 dark:text-gray-400 text-center leading-relaxed mt-4">
                ...
                <div className="space-y-6">
                  <div className="glass border border-white/[0.05] p-6 rounded-2xl">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-white/[0.05] pb-4 mb-6 gap-4">
                      <div>
                        <h3 className="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider">LeadAI Real-Time Event Ledger</h3>
                        <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">Real-time ledger of qualified actions, emails outbox, and call handshakes</p>
                      </div>
                      <span className="text-[10px] font-mono text-gray-500 bg-white/[0.03] px-2.5 py-1 rounded border border-white/[0.05]">
                    ...
                    <div className="space-y-3">
                      {logs.map((log) => (
                        ...
                        >
                          ...
                          <span className={`px-2 py-0.5 text-[9px] font-mono rounded ${log.type === "lead" ? "bg-red-500/10 text-red-400 border border-red-500/20" :
                            log.type === "reply" ? "bg-[#38bdf8]/10 text-[#38bdf8] border border-[#38bdf8]/20" :
                              log.type === "call" ? "bg-amber-500/10 text-amber-400 border border-amber-500/20" :
                                log.type === "workflow" ? "bg-purple-500/10 text-[#33bbcf] border border-purple-500/20" :
              ...
              {activeTab === "calls" && (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div className="glass border border-white/[0.05] p-5 rounded-2xl">
                      <p className="text-gray-600 dark:text-gray-400 text-xs font-semibold">Calls Ingested</p>
                      <h4 className="text-2xl font-bold font-mono text-gray-900 dark:text-white mt-1">18,340</h4>
                      <p className="text-[9px] font-mono text-green-400 mt-2 flex items-center gap-0.5">
                        <ArrowUpRight className="w-3 h-3" /> +8.1% MoM
                      </p>
                    </div>
                    <div className="glass border border-white/[0.05] p-5 rounded-2xl">
                      <p className="text-gray-600 dark:text-gray-400 text-xs font-semibold">Call Success Rate</p>
                      <h4 className="text-2xl font-bold font-mono text-gray-900 dark:text-white mt-1">42.1%</h4>
                      <p className="text-[9px] font-mono text-gray-500 mt-2">Meetings booked success</p>
                    </div>
                    <div className="glass border border-white/[0.05] p-5 rounded-2xl">
                      <p className="text-gray-600 dark:text-gray-400 text-xs font-semibold">Average Call Duration</p>
                      <h4 className="text-2xl font-bold font-mono text-gray-900 dark:text-white mt-1">2m 45s</h4>
                      <p className="text-[9px] font-mono text-gray-500 mt-2">AI conversational length</p>
                    </div>
                    <div className="glass border border-white/[0.05] p-5 rounded-2xl">
                      <p className="text-gray-600 dark:text-gray-400 text-xs font-semibold">Meetings Scheduled</p>
                      <h4 className="text-2xl font-bold font-mono text-gray-900 dark:text-white mt-1">7,820</h4>
                      <p className="text-[9px] font-mono text-green-400 mt-2 flex items-center gap-0.5">
                  ...
                  {/* Call Logs Table */}
                  <div className="glass border border-white/[0.05] p-6 rounded-2xl">
                    <h3 className="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider mb-4">Live Vapi Calling Node Transcripts</h3>
                    <div className="overflow-x-auto">
                      <table className="w-full text-left border-collapse text-xs">
                        <thead>
                          <tr className="border-b border-white/[0.05] text-gray-600 dark:text-gray-400 font-mono uppercase text-[10px]">
                        ...
                        <tbody className="divide-y divide-white/[0.03]">
                          {RECENT_CALLS.map((call) => (
                            <tr key={call.id} className="hover:bg-white/[0.01]">
                              <td className="py-4 text-gray-900 dark:text-white font-medium">{call.customer}</td>
                              <td className="py-4 text-gray-300 font-medium">{call.leadName}</td>
                              <td className="py-4 text-gray-500 font-mono">{call.phone}</td>
                              <td className="py-4 text-gray-600 dark:text-gray-400 font-mono">{call.duration}</td>
                              <td className="py-4">
                                <span className={`px-2.5 py-0.5 rounded text-[10px] font-bold ${call.status === "Booked" ? "bg-green-500/10 text-green-400 border border-green-500/20" :
                ...
                <div className="space-y-6">
                  <div className="glass border border-white/[0.05] p-6 rounded-2xl">
                    <div className="flex items-center justify-between border-b border-white/[0.05] pb-4 mb-6">
                      <div>
                        <h3 className="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider">Active automation workflow loops</h3>
                        <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">Status of system workflows run on LeadAI</p>
                      </div>
                      ...
                      {workflows.map((wf) => (
                        ...
                        >
                          <div className="flex-1">
                            <div className="flex items-center gap-3">
                              <h4 className="text-sm font-bold text-gray-900 dark:text-white">{wf.name}</h4>
                            ...
                            <div className="flex items-center gap-6 mt-3 text-[11px] font-mono text-gray-500">
                              ...
                              <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5 text-[#33bbcf]" /> Last Active: {wf.lastRun}</span>
                            </div>
                          </div>

                          <div className="flex items-center gap-4 justify-between border-t border-white/[0.04] md:border-none pt-4 md:pt-0">
                            <span className={`px-2.5 py-0.5 rounded text-[10px] font-bold ${wf.status === "Running" ? "bg-green-500/10 text-green-400 border border-green-500/20" : "bg-gray-500/10 text-gray-600 dark:text-gray-400 border border-gray-500/20"
                              }`}>
                              {wf.status}
                            </span>
                            <button
                              ...
                              className={`p-2 rounded-lg border transition-all ${wf.status === "Running"
              ...
              {activeTab === "subscriptions" && (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div className="glass border border-white/[0.05] p-5 rounded-2xl">
                      <p className="text-gray-600 dark:text-gray-400 text-xs font-semibold">Free Users</p>
                      <h4 className="text-2xl font-bold font-mono text-gray-900 dark:text-white mt-1">210</h4>
                      <p className="text-[9px] font-mono text-gray-500 mt-2">Active sandbox access</p>
                    </div>
                    <div className="glass border border-white/[0.05] p-5 rounded-2xl">
                      <p className="text-gray-600 dark:text-gray-400 text-xs font-semibold">Paid Active Users</p>
                      <h4 className="text-2xl font-bold font-mono text-gray-900 dark:text-white mt-1">632</h4>
                      <p className="text-[9px] font-mono text-[#38bdf8] mt-2">75.0% Conversion ratio</p>
                    </div>
                    <div className="glass border border-white/[0.05] p-5 rounded-2xl">
                      <p className="text-gray-600 dark:text-gray-400 text-xs font-semibold">Total Revenue (MRR)</p>
                      <h4 className="text-2xl font-bold font-mono text-gray-900 dark:text-white mt-1">$42,500</h4>
                      <p className="text-[9px] font-mono text-green-400 mt-2 flex items-center gap-0.5">
                        <ArrowUpRight className="w-3 h-3" /> +15.4% MoM
                      </p>
                    </div>
                    <div className="glass border border-white/[0.05] p-5 rounded-2xl">
                      <p className="text-gray-600 dark:text-gray-400 text-xs font-semibold">Annualized Run Rate</p>
                      <h4 className="text-2xl font-bold font-mono text-gray-900 dark:text-white mt-1">$510,000</h4>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="glass border border-white/[0.05] p-6 rounded-2xl flex flex-col justify-between">
                      <div>
                        <h3 className="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider mb-6">Subscription Plan Distribution</h3>
                        <div className="space-y-4">
                          {[
                    ...
                    {/* Recent Payments */}
                    <div className="lg:col-span-2 glass border border-white/[0.05] p-6 rounded-2xl">
                      <h3 className="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider mb-4">Recent Invoices Handshakes</h3>
                      <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse text-xs">
                          <thead>
                            <tr className="border-b border-white/[0.05] text-gray-600 dark:text-gray-400 font-mono uppercase text-[10px]">
                          ...
                          <tbody className="divide-y divide-white/[0.03]">
                            {RECENT_PAYMENTS.map((p) => (
                              <tr key={p.id} className="hover:bg-white/[0.01]">
                                <td className="py-3.5 text-gray-600 dark:text-gray-400 font-mono text-[10px]">{p.id.toUpperCase()}</td>
                                <td className="py-3.5 text-gray-900 dark:text-white font-medium">{p.customer}</td>
                                <td className="py-3.5">
                                  <span className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold ${p.plan === "Enterprise" ? "bg-purple-500/10 text-[#33bbcf] border border-purple-500/20" :
                                    "bg-[#38bdf8]/10 text-[#38bdf8] border border-[#38bdf8]/20"
                                    }`}>
                                    {p.plan}
                                  </span>
                                </td>
                                <td className="py-3.5 text-gray-900 dark:text-white font-mono font-semibold">{p.amount}</td>
              ...
              {activeTab === "analytics" && (
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-between border-b border-white/[0.05] pb-4">
                    <h3 className="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider">LeadAI Performance Deep-Dive</h3>
                    <div className="flex items-center gap-1.5 p-1 bg-white/[0.03] border border-white/[0.06] rounded-xl text-[10px] font-mono">
                      {["7d", "30d", "90d", "All"].map(range => (
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-lg transition-all ${timeRange === range ? "bg-white/[0.06] text-[#38bdf8] font-bold" : "text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:text-white"
                            }`}
                        >
                          {range.toUpperCase()}
                        </button>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="glass border border-white/[0.05] p-6 rounded-2xl">
                      <h3 className="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider mb-6">MRR Revenue Growth ($ USD)</h3>
                      <div className="h-64">
                    ...
                    <div className="glass border border-white/[0.05] p-6 rounded-2xl">
                      <h3 className="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider mb-6">Customer Ingestion Rate</h3>
                      <div className="h-64">
                        <svg viewBox="0 0 500 240" className="w-full h-full">
                          <line x1="40" y1="40" x2="480" y2="40" stroke="rgba(255,255,255,0.02)" strokeWidth="1" />
                          <line x1="40" y1="90" x2="480" y2="90" stroke="rgba(255,255,255,0.02)" strokeWidth="1" />
              ...
              {activeTab === "tickets" && (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="glass border border-white/[0.05] p-5 rounded-2xl">
                      <p className="text-gray-600 dark:text-gray-400 text-xs font-semibold">Open Support Tickets</p>
                      <h4 className="text-2xl font-bold font-mono text-gray-900 dark:text-white mt-1">2</h4>
                      <p className="text-[9px] font-mono text-red-400 mt-2">Requires admin follow-up</p>
                    </div>

                    <div className="glass border border-white/[0.05] p-5 rounded-2xl">
                      <p className="text-gray-600 dark:text-gray-400 text-xs font-semibold">Resolved (Last 24h)</p>
                      <h4 className="text-2xl font-bold font-mono text-gray-900 dark:text-white mt-1">12</h4>
                      <p className="text-[9px] font-mono text-green-400 mt-2">Resolved rate: 98%</p>
                    </div>

                    <div className="glass border border-white/[0.05] p-5 rounded-2xl">
                      <p className="text-gray-600 dark:text-gray-400 text-xs font-semibold">Average Response Time</p>
                      <h4 className="text-2xl font-bold font-mono text-gray-900 dark:text-white mt-1">18 mins</h4>
                  ...
                  {/* List of tickets */}
                  <div className="glass border border-white/[0.05] p-6 rounded-2xl">
                    <h3 className="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider mb-6">Support ledger & customer requests</h3>
                    <div className="space-y-4">
                      {SUPPORT_TICKETS.map((t) => (
                        <div key={t.id} className="card-glass flex flex-col">
                          <div className="flex-1">
                            <div className="flex items-center gap-3">
                              <h4 className="text-sm font-bold text-gray-900 dark:text-white">{t.subject}</h4>
                              <span className={`text-[9px] font-mono font-bold px-2 py-0.5 rounded ${t.status === "Open" ? "bg-red-500/10 text-red-400 border border-red-500/20" : "bg-green-500/10 text-green-400 border border-green-500/20"
                                }`}>
                                {t.status}
                              </span>
                              <span className={`text-[9px] font-mono font-bold px-2 py-0.5 rounded ${t.priority === "High" ? "bg-purple-500/10 text-[#33bbcf]" : "bg-gray-500/10 text-gray-600 dark:text-gray-400"
                                }`}>
                                {t.priority} Priority
                              </span>
                            </div>
                            <p className="text-xs text-gray-600 dark:text-gray-400 mt-2 leading-relaxed italic">"{t.message}"</p>
                      ...
                      <div key={idx} className="glass border border-white/[0.05] p-6 rounded-2xl flex flex-col justify-between h-56 hover:border-white/[0.1] transition-all relative overflow-hidden group">
                        ...
                        <div>
                          <div className="flex items-center justify-between border-b border-white/[0.05] pb-3 mb-4">
                            <h4 className="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider">{integ.name}</h4>
                            <span className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold ${integ.connected ? "bg-green-500/10 text-green-400 border border-green-500/20" : "bg-red-500/10 text-red-400 border border-red-500/20"
                              }`}>
                              {integ.status}
                            </span>
                          </div>
                          <p className="text-xs text-gray-600 dark:text-gray-400 leading-relaxed font-sans">{integ.text}</p>
                        </div>
                        ...
                        <div className="border-t border-white/[0.04] pt-4 mt-4 flex items-center justify-between">
                          ...
                          <button
                            onClick={() => {
                              const toast = document.createElement("div");
                              toast.className = "fixed bottom-5 right-5 z-50 glass border border-[#38bdf8]/30 px-4 py-2.5 rounded-xl text-xs flex items-center gap-2 shadow-2xl animate-bounce";
                              toast.innerHTML = `<span class="w-2 h-2 rounded-full bg-green-400 animate-pulse"></span> <span class="text-gray-900 dark:text-white">${integ.name} sync re-scheduled</span>`;
                              document.body.appendChild(toast);
                              setTimeout(() => toast.remove(), 2500);
                            }}
                            className="text-xs text-[#38bdf8] hover:text-gray-900 dark:text-white font-medium transition-colors"
                          >
                  ...
                  <div className="glass border border-white/[0.05] p-6 rounded-2xl">
                    <div className="border-b border-white/[0.05] pb-4 mb-6">
                      <h3 className="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider flex items-center gap-2">
                        ...
                        LeadAI Settings Configuration
                      </h3>
                      <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">Configure global AI LLM defaults and key endpoints</p>
                    </div>
                    ...
                    <form onSubmit={handleSaveSettings} className="space-y-6">
                      <div className="space-y-2">
                        ...
                        <select
                          ...
                          className="w-full bg-white/[0.03] border border-white/[0.06] focus:border-[#38bdf8]/50 text-xs text-gray-900 dark:text-white rounded-xl p-3 outline-none cursor-pointer"
                        >
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          <input
                            ...
                            className="w-full bg-white/[0.03] border border-white/[0.06] focus:border-[#38bdf8]/50 outline-none rounded-xl p-3 text-xs text-gray-900 dark:text-white font-mono"
                          />
            ...
            >
              ...
              <div className="flex items-center justify-between border-b border-white/[0.05] pb-4 mb-4">
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg hover:bg-white/[0.05] text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:text-white transition-colors"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              {modalType === "view_customer" && selectedItem && (
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-[#38bdf8] to-[#6366f1] flex items-center justify-center text-gray-900 dark:text-white font-bold text-sm">
                      {selectedItem.company.slice(0, 2).toUpperCase()}
                    </div>
                    <div>
                      <h5 className="font-bold text-gray-900 dark:text-white text-sm">{selectedItem.company}</h5>
                  ...
                  <div className="grid grid-cols-2 gap-4 border-t border-b border-white/[0.04] py-4 my-2 text-xs">
                    <div>
                      ...
                      <p className="text-gray-900 dark:text-white mt-0.5">{selectedItem.name}</p>
                      <p className="text-[10px] text-gray-600 dark:text-gray-400 font-mono mt-0.5">{selectedItem.email}</p>
                    </div>
                    <div>
                      <p className="text-gray-500">Subscription Tier</p>
                      <p className="text-gray-900 dark:text-white font-semibold mt-0.5">{selectedItem.plan} Tier</p>
                    ...
                    <div>
                      ...
                      <p className="text-gray-900 dark:text-white mt-0.5 flex items-center gap-1.5">
                    ...
                    <div>
                      ...
                      <p className="text-gray-900 dark:text-white font-mono font-bold mt-0.5">{selectedItem.apiCalls.toLocaleString()} calls</p>
                    </div>
                  </div>

                  <div className="space-y-2">
                    <p className="text-xs text-gray-600 dark:text-gray-400">Monthly Usage Threshold Allocation</p>
                    <div className="card-glass flex flex-col">
              ...
              {modalType === "edit_customer" && selectedItem && (
                <div className="space-y-4">
                  <div className="space-y-1">
                    <label className="text-xs text-gray-600 dark:text-gray-400">Workspace Customer Name</label>
                    <input
                      type="text"
                      value={editName}
                      onChange={(e) => setEditName(e.target.value)}
                      className="w-full bg-white/[0.02] border border-white/[0.06] focus:border-[#38bdf8]/50 outline-none rounded-xl p-2.5 text-xs text-gray-900 dark:text-white"
                    />
                  </div>

                  <div className="space-y-1">
                    <label className="text-xs text-gray-600 dark:text-gray-400">Company Brand</label>
                    <input
                      type="text"
                      value={editCompany}
                  ...
                  <div className="space-y-1">
                    <label className="text-xs text-gray-600 dark:text-gray-400">Workspace Email Address</label>
                    <input
                      type="text"
                      value={editEmail}
                  ...
                  <div className="space-y-1">
                    <label className="text-xs text-gray-600 dark:text-gray-400">Subscription Plan Tier</label>
                    <select
                      value={editPlan}
                  ...
                  <div className="border-t border-white/[0.05] pt-4 mt-6 flex justify-end gap-3">
                    <button
                      onClick={() => setModalType(null)}
                      className="px-4 py-2 border border-white/[0.05] hover:bg-white/[0.03] text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:text-white rounded-xl text-xs transition-colors"
                    >
                      Cancel
                    </button>
                    <button
                ...
                <div className="space-y-4">
                  ...
                  <div className="border-t border-white/[0.05] pt-4 mt-6 flex justify-end gap-3">
                    ...
                    <button
                      ...
                      className={`px-5 py-2 text-black font-semibold rounded-xl text-xs transition-all ${selectedItem.status === "Active" ? "bg-red-500 hover:bg-red-400" : "bg-green-500 hover:bg-green-400"
                        }`}
                    >
                ...
                <div className="space-y-4">
                  <div className="space-y-1.5 text-xs">
                    <div className="flex border-b border-white/[0.04] pb-2">
                      ...
                      <span className="text-gray-900 dark:text-white font-mono">{selectedItem.leadEmail}</span>
                    </div>
                    <div className="flex border-b border-white/[0.04] py-2">
                      <span className="text-gray-500 w-16">Subject:</span>
                      <span className="text-gray-900 dark:text-white font-medium">{selectedItem.subject}</span>
                    </div>
                    <div className="flex border-b border-white/[0.04] py-2">
                  ...
                  <div className="p-3 bg-[#38bdf8]/5 border border-[#38bdf8]/10 text-[#38bdf8] rounded-xl flex items-center gap-2 text-[10px] leading-relaxed">
                    ...
                    <span>LeadAI created this response draft using LLM synthesis based on ingest documents.</span>
                  </div>
                  ...
                  <div className="border-t border-white/[0.05] pt-4 mt-6 flex justify-end gap-2">
                    <button
                      onClick={() => {
                        ...
                        toast.innerHTML = `<span class="text-red-400">●</span> <span class="text-gray-900 dark:text-white">Draft discarded</span>`;
                        document.body.appendChild(toast);
                        setTimeout(() => toast.remove(), 2500);
                        setModalType(null);
                      }}
                    ...
                    <button
                      onClick={() => {
                        ...
                        toast.innerHTML = `<span class="text-green-400">●</span> <span class="text-gray-900 dark:text-white">Email submitted successfully</span>`;
                        document.body.appendChild(toast);
                        setTimeout(() => toast.remove(), 2500);
                        setModalType(null);
                      }}
              ...
              {modalType === "call_transcript" && selectedItem && (
                <div className="space-y-4">
                  <div className="flex justify-between items-center text-xs">
                    <div>
                      <p className="text-gray-900 dark:text-white font-bold">{selectedItem.leadName}</p>
                  ...
                  <div className="card-glass flex flex-col">
                    <span className="text-[10px] text-gray-600 dark:text-gray-400 font-mono">Audio Recording:</span>
                    ...
                    <button className="p-1 text-[#38bdf8] hover:text-gray-900 dark:text-white transition-colors bg-[#38bdf8]/10 rounded border border-[#38bdf8]/20">
                  ...
                  <div className="border-t border-white/[0.05] pt-4 mt-6 flex justify-end">
                    <button
                      onClick={() => setModalType(null)}
                      className="px-5 py-2 border border-white/[0.05] hover:bg-white/[0.03] text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:text-white rounded-xl text-xs transition-colors"
                    >
                      Close Transcripts
                    </button>
                  </div>
              ...
              {modalType === "ticket_reply" && selectedItem && (
                <div className="space-y-4">
                  <div>
                    <h5 className="font-bold text-gray-900 dark:text-white text-sm">{selectedItem.subject}</h5>
                  ...
                  <div className="space-y-2">
                    <label className="text-xs text-gray-600 dark:text-gray-400">Administrative Response</label>
                    <textarea
                      rows="4"
                      value={ticketReplyText}
                      onChange={(e) => setTicketReplyText(e.target.value)}
                      placeholder="Write your response to the customer..."
                      className="w-full bg-white/[0.03] border border-white/[0.06] focus:border-[#38bdf8]/50 outline-none rounded-xl p-3 text-xs text-gray-900 dark:text-white font-sans resize-none"
                    />
                  </div>
