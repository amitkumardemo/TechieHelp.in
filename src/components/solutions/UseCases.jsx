...
const useCases = [
  ...
  },
  {
    ...
    icon: <Rocket className="w-10 h-10 text-[#33bbcf]" />,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {useCases.map((item, i) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="group p-10 rounded-[3.5rem] bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 hover:border-gray-300 dark:border-white/20 transition-all duration-700 backdrop-blur-2xl flex flex-col md:flex-row gap-8 items-center md:items-start"
            >
              <div className="w-24 h-24 shrink-0 rounded-[2rem] bg-gray-100 dark:bg-white/5 flex items-center justify-center group-hover:scale-110 group-hover:rotate-6 transition-all duration-500 border border-white/5 shadow-xl">
                {item.icon}
              </div>
              <div className="flex-1">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-2xl font-black text-gray-900 dark:text-white uppercase tracking-tighter">{item.title}</h3>
                  <motion.div whileHover={{ scale: 1.2 }} className="text-gray-900 dark:text-white/20 group-hover:text-gray-900 dark:text-white transition-colors cursor-pointer">
                    <ArrowUpRight className="w-6 h-6" />
                  </motion.div>
                </div>
                <p className="text-gray-600 dark:text-gray-400 text-sm leading-relaxed mb-6 font-bold italic opacity-80 group-hover:opacity-100 transition-opacity">
                  {item.desc}
                </p>
