...
const items = [
  ...
  { title: "AI-First Engine", desc: "Custom-trained LLM logic for business-specific reasoning.", icon: <Cpu className="w-8 h-8 text-purple-400" /> },
  { title: "Fully Automated", desc: "Our systems run even when you are offline, handling thousands of requests.", icon: <Zap className="w-8 h-8 text-yellow-500" /> },
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
          {items.map((item, i) => (
            <motion.div
              ...
              className="p-8 rounded-[2.5rem] bg-white/5 border border-white/10 hover:border-blue-500/30 transition-all duration-500 flex flex-col items-center text-center group backdrop-blur-xl hover:bg-white/10"
            >
              <div className="mb-6 p-4 rounded-2xl bg-white/5 group-hover:bg-blue-500/10 group-hover:rotate-12 transition-all">
                {item.icon}
              </div>
              <h3 className="text-sm font-black text-white mb-3 uppercase tracking-tighter leading-tight">{item.title}</h3>
