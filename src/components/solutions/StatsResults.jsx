...
const stats = [
  ...
  { label: "24/7 Operations", desc: "Fully Autonomous", icon: <Clock8 className="w-8 h-8 text-[#33bbcf]" />, shadow: "shadow-[0_0_30px_rgba(168,85,247,0.1)]" }
];
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {stats.map((stat, i) => (
            <motion.div
              ...
              className={`p-10 rounded-[3rem] bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 hover:border-gray-300 dark:border-white/20 transition-all duration-500 text-center flex flex-col items-center group ${stat.shadow}`}
            >
              <div className="mb-8 p-6 rounded-[2rem] bg-gray-100 dark:bg-white/5 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500">
                {stat.icon}
              </div>
              <h3 className="text-4xl font-black text-gray-900 dark:text-white mb-2 uppercase tracking-tighter leading-none">{stat.label}</h3>
              <p className="text-[10px] font-black text-gray-500 uppercase tracking-[4px] mt-4">{stat.desc}</p>
              
              {/* Decorative progress line mockup */}
              <div className="w-full h-[1px] bg-gray-100 dark:bg-white/5 mt-10 relative overflow-hidden">
