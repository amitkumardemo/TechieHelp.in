        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {plans.map((plan, i) => (
            <motion.div
              ...
              whileHover={{ y: -10 }}
              className={`relative flex flex-col p-8 rounded-32 border h-full transition-all duration-500 ${
                plan.popular 
                ? 'bg-gradient-to-b from-blue-600/20 to-primary border-blue-500 shadow-[0_20px_50px_rgba(59,130,246,0.2)]' 
                : 'bg-white/5 border-white/10 hover:border-white/20'
              }`}
            >
              {plan.popular && (
                <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 px-4 py-1.5 bg-blue-500 text-white rounded-full text-xs font-black uppercase tracking-widest whitespace-nowrap">
                  Most Popular
                </div>
              )}
              ...
              <div className="flex-1 space-y-4 mb-10">
                {plan.features.map((feature, j) => (
                  <div key={j} className="flex items-start gap-3">
                    <div className={`mt-1 p-0.5 rounded-full ${plan.popular ? 'bg-blue-500' : 'bg-white/10'}`}>
