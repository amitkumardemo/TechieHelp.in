...
const InboxMockup = () => (
  <div className="w-full h-full bg-[#f9fafb] flex">
    <div className="w-52 bg-white border-r border-gray-100 p-4 flex flex-col gap-1.5 shrink-0">
      <div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3">Inbox · 5 new</div>
      {[
        ...
        <div key={i} className={`flex items-start gap-2.5 p-2.5 rounded-xl cursor-pointer transition-all ${item.active ? "bg-blue-50 border border-blue-100" : "hover:bg-gray-50"}`}>
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[11px] text-gray-400 truncate">{item.msg}</p>
          </div>
    ...
    <div className="flex-1 flex flex-col p-5 gap-3">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-bold text-gray-900">Rahul Sharma · TechNova</h3>
          <p className="text-xs text-gray-400">rahul@technova.com · LinkedIn Ad</p>
        </div>
        <div className="flex gap-1.5">
          <span className="px-2.5 py-1 bg-red-100 text-red-600 text-[10px] font-bold rounded-full">HOT</span>
          <span className="px-2.5 py-1 bg-blue-100 text-blue-600 text-[10px] font-bold rounded-full">Score 92</span>
        </div>
      </div>
      <div className="flex-1 bg-white rounded-2xl border border-gray-100 p-4 flex flex-col gap-3">
        <div className="flex gap-2.5">
          <div className="w-7 h-7 rounded-full bg-gray-100 flex items-center justify-center text-[10px] font-bold text-gray-500 shrink-0">RS</div>
          <div className="bg-gray-50 rounded-2xl rounded-tl-none px-3.5 py-2.5 max-w-[75%]">
            <p className="text-[12px] text-gray-700">Hi, I'm looking for an AI solution to automate our lead management. Budget is ₹50K+.</p>
          </div>
        </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.1 }} className="flex items-center gap-2 pl-9">
          {[0, 0.18, 0.36].map((d, i) => (
            <motion.div key={i} animate={{ y: [0, -3, 0] }} transition={{ repeat: Infinity, duration: 0.7, delay: d }} className="w-1.5 h-1.5 rounded-full bg-gray-300" />
          ))}
          <span className="text-[11px] text-gray-400 ml-1">LeadAI is typing…</span>
        </motion.div>
      </div>
    </div>
  </div>
...
const MemoryMockup = () => (
  <div className="w-full h-full bg-[#f9fafb] flex">
    <div className="w-48 bg-white border-r border-gray-100 p-4 flex flex-col gap-1 shrink-0">
      <div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3">Customers</div>
      {["Rahul Sharma", "Priya Desai", "Sneha Reddy", "Amit Kumar", "Vikram Singh"].map((name, i) => (
        <div key={i} className={`flex items-center gap-2 px-2.5 py-2 rounded-lg text-[12px] font-medium cursor-pointer transition-all ${i === 0 ? "bg-indigo-50 text-indigo-700" : "text-gray-500 hover:bg-gray-50"}`}>
    ...
    <div className="flex-1 p-5 flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">TechNova Solutions · Founder</p>
          </div>
        </div>
        <span className="text-[11px] bg-green-100 text-green-700 px-2.5 py-1 rounded-full font-semibold">Active Client</span>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-4 flex-1">
        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3">AI Memory Timeline</p>
        <div className="space-y-2.5">
          {[
            ...
            <motion.div key={i} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.12 }} className="flex items-start gap-2.5 text-[12px]">
              <span>{ev.icon}</span>
              <span className="text-gray-600 flex-1">{ev.text}</span>
...
const CallingMockup = () => (
  <div className="w-full h-full bg-[#f9fafb] flex flex-col p-5 gap-3">
    <motion.div animate={{ boxShadow: ["0 0 0px rgba(99,102,241,0)", "0 0 25px rgba(99,102,241,0.25)", "0 0 0px rgba(99,102,241,0)"] }} transition={{ repeat: Infinity, duration: 2 }}
      className="bg-indigo-600 rounded-2xl p-4 flex items-center justify-between">
      <div className="flex items-center gap-3">
    ...
    <div className="bg-white rounded-2xl border border-gray-100 flex-1 p-4 flex flex-col gap-2.5 overflow-hidden">
      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Live Transcript</p>
      {[
        ...
        <motion.div key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: line.delay + 0.2 }} className={`flex gap-2 ${line.who !== "AI" ? "justify-end" : ""}`}>
          ...
          <div className={`rounded-2xl px-3 py-2 max-w-[78%] ${line.who === "AI" ? "bg-gray-50 rounded-tl-none" : "bg-indigo-50 rounded-tr-none"}`}>
            <p className="text-[12px] text-gray-700">{line.msg}</p>
          </div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.4 }} className="flex gap-1 items-center pl-8">
        {[0, 0.2, 0.4].map((d, i) => <motion.div key={i} animate={{ y: [0, -3, 0] }} transition={{ repeat: Infinity, duration: 0.7, delay: d }} className="w-1.5 h-1.5 rounded-full bg-gray-300" />)}
        <span className="text-[11px] text-gray-400 ml-1">AI is responding…</span>
      </motion.div>
    </div>
  </div>
);
...
const AnalyticsMockup = () => (
  <div className="w-full h-full bg-[#f9fafb] p-5 flex flex-col gap-3">
    <div className="grid grid-cols-4 gap-2">
      {[
        { label: "Total Revenue", val: "₹12.4L", change: "+24%", color: "text-emerald-600", bg: "bg-emerald-50" },
        { label: "Leads Captured", val: "1,284", change: "+18%", color: "text-blue-600", bg: "bg-blue-50" },
        { label: "Connect Rate", val: "87%", change: "+5%", color: "text-purple-600", bg: "bg-purple-50" },
        { label: "Avg Deal Size", val: "₹82K", change: "+12%", color: "text-amber-600", bg: "bg-amber-50" },
      ].map((m, i) => (
        <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} className="bg-white rounded-2xl border border-gray-100 p-3">
    ...
    <div className="bg-white rounded-2xl border border-gray-100 flex-1 p-4">
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm font-bold text-gray-800">Revenue Pipeline</p>
          ...
          return (
            <div key={month} className="flex-1 flex flex-col items-center gap-1">
              <div className="w-full flex items-end gap-0.5">
                <motion.div initial={{ height: 0 }} animate={{ height: h1[i] }} transition={{ duration: 0.5, delay: i * 0.07 }} className="flex-1 bg-blue-200 rounded-t-md" />
                <motion.div initial={{ height: 0 }} animate={{ height: h2[i] }} transition={{ duration: 0.5, delay: i * 0.07 + 0.08 }} className="flex-1 bg-emerald-400 rounded-t-md" />
              </div>
...
const Hero = () => {
  ...
  return (
    <section id="home" className="relative w-full bg-white pt-28 pb-0 flex flex-col items-center overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none z-0"
        style={{ background: "radial-gradient(ellipse 80% 60% at 50% 0%, transparent 40%, white 100%)" }}
      />
      ...
      <motion.div
        ...
        className="relative z-10 flex items-center gap-1 mb-7"
      >
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 shadow-sm">
          <span className="flex items-center gap-1 text-blue-600 text-[11px] font-bold uppercase tracking-[0.18em]">
      ...
      <motion.div
        ...
        className="relative z-10 text-center px-4 max-w-[960px]"
      >
        <h1 className="font-poppins font-black text-[44px] ss:text-[56px] sm:text-[68px] md:text-[78px] lg:text-[88px] leading-[1.0] tracking-[-0.035em] text-gray-900 dark:text-white">
          ...
          <span className="relative inline-block">
            ...
            >
              <motion.path
                d="M 4 8 Q 100 2 200 8 Q 300 14 396 8"
                fill="none"
      ...
      {/* ── Three trust-signal pills ── */}
      <motion.div
        initial={{ opacity: 0, y: 14 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6, delay: 0.15 }}
        className="relative z-10 flex flex-wrap items-center justify-center gap-2 mt-6 px-4"
      >
        {[
          ...
          <div key={i} className="flex items-center gap-1.5 px-3 py-1 bg-gray-50 border border-gray-100 rounded-full text-gray-500 text-[12px] font-medium">
            <pill.icon className="w-3 h-3 text-gray-400" />
            {pill.label}
          </div>
        ))}
      </motion.div>
      ...
      <motion.p
        ...
        className="relative z-10 mt-6 text-center text-[15px] sm:text-[17px] text-gray-500 max-w-[680px] leading-relaxed px-5"
      >
        LeadAI captures leads, replies instantly, calls customers,
        updates your CRM, and compounds revenue —
        <strong className="text-gray-700 font-semibold"> 24/7, fully automated.</strong>
      </motion.p>
      ...
      >
        <a
          href="/contacts"
          className="relative overflow-hidden group flex items-center gap-2 px-8 py-3.5 bg-black text-white text-[15px] font-bold rounded-full transition-all shadow-lg hover:shadow-[0_0_20px_rgba(51,187,207,0.4)] hover:-translate-y-0.5"
        >
          <span className="absolute inset-0 w-full h-full bg-gradient-to-r from-[#33bbcf] to-[#5ce1e6] opacity-0 group-hover:opacity-100 transition-opacity duration-300"></span>
        ...
        <a
          href="/contacts"
          className="flex items-center gap-2 px-8 py-3.5 border border-gray-200 hover:border-gray-400 bg-white text-gray-700 hover:text-gray-900 text-[15px] font-medium rounded-full transition-all hover:-translate-y-0.5"
        >
          ...
          {TABS.map((tab, index) => {
            ...
            return (
              ...
              >
                ...
                {tab.label}
                {isActive && (
                  <span className="absolute bottom-[-1px] left-0 right-0 h-[2px] bg-gray-100 rounded-full overflow-hidden">
                    <motion.span
      ...
      <motion.div
        ...
        className="relative z-10 w-full max-w-5xl px-4 mt-3"
      >
        <div className="w-full rounded-t-2xl border border-gray-200 border-b-0 overflow-hidden shadow-[0_-4px_40px_rgba(0,0,0,0.06)]">
          {/* Browser top bar */}
          <div className="flex items-center gap-3 px-4 py-3 bg-gray-50 border-b border-gray-200">
            ...
            <div className="flex-1 bg-white border border-gray-200 rounded-md h-6 flex items-center px-3 gap-2">
              <Shield className="w-3 h-3 text-green-500" />
              <span className="text-[11px] text-gray-400">app.techiehelp.in / {TABS[activeIndex].id}</span>
            </div>
          ...
          <div className="relative w-full bg-[#f9fafb] overflow-hidden" style={{ height: 360 }}>
