...
import AIWorkflowAutomation from "./components/solutions/AIWorkflowAutomation";
import AIBusinessDashboard from "./components/solutions/AIBusinessDashboard";
import VoiceAISystem from "./components/solutions/VoiceAISystem";
  ...
  useEffect(() => {
    ...
    if (!isLeadAdmin) {
      ctx = gsap.context(() => {
        ...
        // Animate images and icons on scroll (excluding nav and footer elements)
        gsap.utils.toArray("img:not(nav img):not(footer img), svg:not(nav svg):not(footer svg)").forEach((element) => {
          gsap.from(element, {
            scrollTrigger: {
              trigger: element,
              start: "top 80%",
